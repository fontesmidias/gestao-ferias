      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {tenants.map((t) => {
            ...
            const inactive = (t as any).isActive === false
            return (
              <div key={t.id} className={`glass-card rounded-xl border p-5 flex flex-col gap-3 ${inactive ? 'border-rose-500/20 opacity-60' : 'border-white/5'}`}>
                {/* Header: name + badges + actions icon group */}
                <div className="flex items-start justify-between gap-3">
                  ...
                  {/* Icon-only actions (secondary) */}
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => openEditTenant(t)} title="Editar"
                      className="p-1.5 text-slate-500 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
                      <Edit3 className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => { setSelectedTenantId(t.id); setShowUserModal(true) }} title="Criar Usuario"
                      className="p-1.5 text-slate-500 hover:text-indigo-400 hover:bg-indigo-500/10 rounded-lg transition-colors">
                      <UserPlus className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => deactivateTenant(t.id, inactive)} title={inactive ? 'Reativar' : 'Desativar'}
                      className={`p-1.5 rounded-lg transition-colors ${inactive ? 'text-emerald-500 hover:bg-emerald-500/10' : 'text-slate-500 hover:text-amber-400 hover:bg-amber-500/10'}`}>
                      <Power className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => openDeleteModal(t)} title="Excluir permanentemente"
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-bold text-emerald-400 border border-emerald-500/20 rounded-lg hover:bg-emerald-500/10 transition-colors disabled:opacity-50"
                  >
                  ...
                  <Link
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-bold text-slate-300 border border-white/10 rounded-lg hover:bg-white/5 transition-colors"
                  >
