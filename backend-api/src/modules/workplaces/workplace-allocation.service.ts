...
export interface UpsertFromImportInput {
  ...
  source: string
  /**
   * Quando fornecido, executa dentro deste transactional client em vez de
   * abrir um novo $transaction. Necessário para que importers (que já rodam
   * dentro de uma transação maior) possam chamar este service sem nested
   * $transaction (não suportado pelo Prisma).
   *
   * Story 2.1.
   */
  tx?: TxClient
...
export class WorkplaceAllocationService {
  ...
  async upsertFromImport(input: UpsertFromImportInput): Promise<UpsertResult> {
    if (input.tx) {
      return this.runInTx(input.tx, input)
    }
    return this.prisma.$transaction((tx) =>
      this.runInTx(tx as TxClient, input),
    )
  }

  private async runInTx(
    tx: TxClient,
    input: UpsertFromImportInput,
