...
import { importWorkplaces } from '../../../../modules/workplaces/import-workplaces.service'
...
const workplaces: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
  ...
  fastify.post('/import', {
    ...
    try {
      ...
      const { created, updated, positions, defaultsCreated } =
        await importWorkplaces({ prisma: fastify.prisma, tenantId, rawData })

      const defaultsMsg =
        defaultsCreated > 0 ? `, ${defaultsCreated} posições padrão criadas` : ''
      return {
        message: `Importação concluída: ${created} postos criados, ${updated} atualizados, ${positions} posições adicionadas${defaultsMsg}.`,
        workplaces: created,
        updated,
        positions,
        defaultsCreated,
