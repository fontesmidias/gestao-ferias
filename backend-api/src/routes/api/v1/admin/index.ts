...
const admin: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
  ...
  fastify.post('/master-key/generate', {
    ...
    const newKey = randomBytes(32).toString('hex') // 64 chars hex
    ...
    fastify.log.warn('[MASTERKEY] New MasterKey generated by SuperAdmin')
    return { key: newKey, message: 'Nova MasterKey gerada. Copie e guarde em local seguro. Ela nao sera exibida novamente.' }
  })
